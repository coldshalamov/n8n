...
export function ContractorFilters({ trades }: { trades: string[] }) {
  ...
  const setParam = (key: string, value: string | null) => {
    ...
    start(() => router.replace(`/contractors?${next.toString()}`, { scroll: false }));
  };
      ...
      <div className="flex flex-wrap items-center gap-1.5">
        ...
        {trades.map((t) => (
          <button
            key={t}
            type="button"
            onClick={() => setParam('trade', t === trade ? null : t)}
            className={clsx(
